...
before(function (done) {
  ...
  test.setUpTestEnv((p) => {
    ...
    expect(identifiers).to.have.lengthOf(5)
    done()
  }, 'replicating/replicating.json')
})
...
it('messaging', function (done) {
  ...
  setTimeout(() => {
    _send('inspectJSON', processes['math.ms@127.0.0.1:4000'], (data) => {
      // 4 string + 1 math + tester node
      expect(data.global.systemConf.nodes).to.have.lengthOf(5 + 1)
      done()
    })
  }, 5000)
})
